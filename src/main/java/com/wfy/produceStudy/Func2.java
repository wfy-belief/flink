...
public class Func2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        KeyedStream<Tuple3<String, Double, Integer>, String> cityGroup = Jobs.map(new MapFunction<Job, Tuple3<String, Double, Integer>>() {
            @Override
            public Tuple3<String, Double, Integer> map(Job value) {
                return new Tuple3<>(value.getCity(), value.getSalary(), 1);
            }
        }).keyBy(item -> item.f0);

        SingleOutputStreamOperator<Tuple3<String, Double, Integer>> reduce = cityGroup.reduce(new ReduceFunction<Tuple3<String, Double, Integer>>() {
            @Override
